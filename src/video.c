...
render_line(uint16_t y, float scan_pos_x)
{
	...
	// SDRAM bitmap layer (VERA_2) composited over VERA.  Default: full-screen
	// replacement.  With CTRL[3] passthru: where VERA drew an opaque pixel
	// (col_line != 0 -- a sprite like the mouse, or a layer) VERA shows over the
	// bitmap; the bitmap fills everywhere VERA is transparent.
	if (bitmap2_active()) {
		bool pass = bitmap2_passthru();
		uint32_t* framebuffer4 = framebuffer4_begin;
		for (uint16_t x = s_pos_x_p; x < s_pos_x; x++) {
			if (!(pass && col_line[x] != 0))
				*framebuffer4 = bitmap2_color_at(x, y);
			framebuffer4++;
		}
	}
