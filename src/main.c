...
usage()
{
	...
	printf("-bitmap2\n");
	printf("\tEnable the SDRAM bitmap layer (VERA_2) at $9F60-$9F6F: 640x480 4/8bpp.\n");
	printf("\tX16-MiSTer core extension; do not combine with -midicard at $9F60.\n");
...
main(int argc, char **argv)
{
	...
	while (argc > 0) {
		if (!strcmp(argv[0], "-rom")) {
			...
			argc--;
			argv++;
		} else if (!strcmp(argv[0], "-bitmap2")) {
			// enable the SDRAM bitmap layer (VERA_2) at $9F60-$9F6F
			has_bitmap2 = true;
			argc--;
			argv++;
